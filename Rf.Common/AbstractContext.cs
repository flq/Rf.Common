...
namespace Playground
{
  ...
  /// </summary>
  public class StandardContext : AbstractContext<StandardContext> 
  {
  ...
  public abstract class AbstractContext<TARGET> 
    : IContext<TARGET>, ICloneable, IDisposable where TARGET : AbstractContext<TARGET>
  {
    ...
    /// <param name="extension">The extension which must implement the <see cref="IContextExtension&lt;TARGET&gt;"/> interface</param>
    public virtual void AddExtension<T>(T extension) where T : IContextExtension<TARGET>
    {
    ...
    public virtual void RemoveExtension<T>() where T : IContextExtension<TARGET>
    {
      if (!attachedObjects.ContainsKey(typeof(T))) return;
      Remove((IContextExtension<TARGET>)attachedObjects[typeof(T)].TheObject);
    }
    ...
    public virtual void Remove(IContextExtension<TARGET> extension)
    {
      var ext = attachedObjects.Values.Where(e => e.TheObject == extension).FirstOrDefault();
      if (ext == null) return;
      ((IContextExtension<TARGET>)ext.TheObject).Remove((TARGET)this);
    ...
    protected void ForceAdd<T>(T @object)
    {
      ...
      if ((@object != null) && @object.GetType().ImplementsInterface<IContextExtension<TARGET>>())
      {
        ((IContextExtension<TARGET>)@object).Attach((TARGET)this);
      }
    }
    ...
    /// <summary>
    /// <see cref="IContext{TARGET}.WhatDoIHave()"/>
    /// </summary>
    ...
    public void Dispose()
    {
      ...
      {
        var extension = o.TheObject as IContextExtension<TARGET>;
        if (extension != null)
